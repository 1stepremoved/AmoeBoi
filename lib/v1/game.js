...
class Game {
  ...
  animate = () => {
    ...
    if (this.paused) {
      this.amoebas.forEach(amoeba => {
        amoeba.draw(this.ctx, this.boardVars);
      });
      this.amoeboi.draw(this.ctx, this.boardVars);
  ...
  moveAmoebas = (ctx) => {
    ...
    this.amoeboi ? this.amoeboi.draw(this.ctx, this.boardVars) : null;
    ctx.globalAlpha = 1;
    this.quadTree.checkAllCollisions(aabbCheck);
    this.amoebas.forEach(amoeba => {
      this.amoeboi ? aabbCheck(this.amoeboi, amoeba) : null;
      this.amoeboi ? aabbCheck(amoeba, this.amoeboi) : null;
